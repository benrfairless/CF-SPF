      ...
      try {
        ...
        if (!domain) {
          return new Response(JSON.stringify({ 
            ...
            usage: 'GET /flatten?domain=example.com or POST /flatten with {"domain": "example.com"}'
          }), {
            status: 400,
            headers: {
              'Content-Type': 'application/json',
        ...
        const flattenedSPF = await flattener.flatten(domain);

        return new Response(JSON.stringify({
          domain: domain,
          original_spf: await flattener.resolveTXT(domain),
          flattened_spf: flattenedSPF,
